...
public class UserLoginServiceImpl implements UserLoginService {
	...
	public UserLoginDTO userLogin(String username, String password, String usertype) throws AtAppException {
		...
		if(!userInfo.getUname().equals(username.trim()) || !userInfo.getPassword().equals(password.trim())) {
			throw new AtAppException("UserDetails not found", HttpStatus.NOT_FOUND);
		}
		...
		if(null != unizenKeyConfig && (unizenKeyConfig.getIsEnabled() != null && 
			...
			userLoginDTO.setEmailId(userInfo.getEmailId());
			userLoginDTO.setContactNumber(userInfo.getContactnumber());
			userLoginDTO.setUserName(userInfo.getUname());
			userLoginDTO.setAppId(userInfo.getUserDeviceMappings().get(0).getAppId());
	...
	public List<UserDeviceMapping> getUserDeviceOrgId(String userId) throws AtAppException {
		// TODO Auto-generated method stub
		return userDeviceMappingDao.getUserDeviceOrgId(userId);
	}
	...
	public void changePwd(TblUserInfo userInfo) throws AtAppException {
		userInfoDao.save(userInfo);		
	}
	...
	public TblUserInfo setPwd(TblUserInfo user) throws AtAppException {		
		return userInfoDao.save(user);
	}



	public TblPushNotifications getNotificationInfoByUserId(String userId) throws Exception {
			return pushNotificationsDao.getNotificationInfoByUserId(userId);
	}



	
	public void persistPushNotification(TblPushNotifications ps) throws Exception {
		pushNotificationsDao.save(ps);
	}



	public List<TblHydacInfo> getHydacInfos() throws Exception {
		return hydacDao.getHydacInfos();
	}
