...
public interface FrameDao extends JpaRepository<LoraFrame, Serializable> {
	...
	*/
	...
	/*changes due to retry consoildated*/
	...
	@Query(value="SELECT sum(f.waterltr) as total_waterltr FROM lora_frames f where f.applicationID=?1 and f.devEUI=?2 and f.devMapId between ?3 and ?4",nativeQuery=true)
	Long getWaterConsumptionsUnitForEndUserOnConsoilID(@Param("appId") String appId,@Param("devEUI") String devEUI,@Param("startFrm") long startFrm, @Param("endFrm") long endFrm);
	...
	@Query(value = "CALL frames_weekly_interval(?1,?2,?3,?4,?5)", nativeQuery = true)
	Object[] getTotalWeeklyIntervalWaterConsumptions(@Param("reqDt") Date reqDt,@Param("eDt") Date eDt ,@Param("appId") String appId,@Param("devEUI") String devEUI,@Param("interval") String interval);

	@Query(value="SELECT f.* FROM lora_frames f where f.devEUI=?1 and f.applicationID=?2  and f.devMapId between ?3 and ?4 order by f.id desc limit 1 ",nativeQuery=true)
	LoraFrame getLoraFrameByDevEUIAndAppIDAndStartEndRange(@Param("devEUI") String devEUI, @Param("appId") String appId, @Param("startFrm") long startFrm, @Param("endFrm") long endFrm);
