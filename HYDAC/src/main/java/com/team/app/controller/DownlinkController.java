...
public class DownlinkController {
	...
	/*@RequestMapping(value= {"/deleteDownlinkQueue"}, method=RequestMethod.GET)
    public String deleteDownlinkQueueHandler(HttpServletRequest request,HttpSession session,Map<String,Object> map,RedirectAttributes redirectAttributes) {
		 logger.debug("/inside deleteDownlinkQueue");	
		   try{
				Map<String,Object> orgMapped=organisationService.getLoraServerOrganisation();	   
		   			map.put("organisations", orgMapped);
	...
	@RequestMapping(value= {"/allInOneDownlink"}, method=RequestMethod.GET)
	public String allInOneDownlinkHanlder(HttpSession session,HttpServletRequest request,Map<String,Object> map) {
		logger.debug("Inside /allInOneDownlink");			
		try{	
			Map<String,Object> orgMapped=organisationService.getLoraServerOrganisation();	   
				map.put("organisations", orgMapped);
			List<TblKeywordType> keyTypes= keywordService.getKeywordTypes(); 
				map.put("keyTypes",keyTypes);
					 return "allInOneDownlink";
		}catch(Exception e){
			e.printStackTrace();
			HttpSession s=request.getSession();
		    s.setAttribute("statusLog",AppConstants.statusLog);
	...
	@RequestMapping(value= {"/allInOneSend"}, method=RequestMethod.POST)
	public @ResponseBody String allInOneSendHandler(HttpSession session,HttpServletRequest request,Map<String,Object> map) throws Exception{
		logger.debug("Inside /allInOneSend");
		String orgs=request.getParameter("orgId").trim();
		String[] orgArr=orgs.split(":");
		String orgId=orgArr[0];
		String apps=request.getParameter("appId").trim();
		...
		try{			
			LoraFrame frm=consumerInstrumentService.getLoraFrameByDevEUIAndAppID(devId,appId);
			if(frm!=null){					
					returnVal=downlinkService.doDownlinkForAllInOne(frm.getDevEUI(), frm.getApplicationID(), frm.getfPort());				
			}else{
				logger.debug("No Data Found!");
				returnVal="No Data Found!";
			}	
	...
	public String dateConfigHandler(HttpSession session,HttpServletRequest request,Map<String,Object> map) {
		logger.debug("Inside /downlinkQueue Handler");	
		try{
				String devEUI=request.getParameter("devid");
				String appIdName=request.getParameter("appid");
