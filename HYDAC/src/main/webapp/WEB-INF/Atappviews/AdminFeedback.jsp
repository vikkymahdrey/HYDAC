...
<body class="hold-transition skin-blue skin-black sidebar-mini"
	...
	String success = (String) session.getAttribute("success");
		...
		if (success == null) {
			...
			success = ("<div id='myModal' class='modal fade' data-keyboard='false' data-backdrop='static'>" + "<div class='modal-dialog'>"
					...
					+ "<div class='modal-body text-center' style='background-color:#f9f9f9'>Issue has been updated as "+ status + "</div>"
	...
	<%
		...
		if (fail == null) {
			...
			fail = ("<div id='myModal' class='modal fade' data-keyboard='false' data-backdrop='static'>" + "<div class='modal-dialog'>"
					+ " <div class='modal-content'>" + "<div class='modal-header'style='background-color:#235E93;color:#ffff'>"
					+ " <button type='button' class='pull-right' data-dismiss='modal' aria-label='Close' style='background:none;border:0'>"
			        + "<span aria-hidden='true' class='glyphicon glyphicon-remove'></span></button>"
					+ " <h4 class='modal-title text-center' >Failure message</h4>" + "</div>"
					+ "<div class='modal-body text-center' style='background-color:#f9f9f9'>Unable to resolve the issue, please try again" + "</div>"
					+ "<div class='modal-footer' style='background-color:#f9f9f9'>"
					+ "<button type='button' class='btn btn-info' data-dismiss='modal'style='background-color:#64B246;box-shadow: 0px 0px 2px 2px rgba(0,0,0,0.10);border:0'>OK</button>"
					+ "</div>" + "</div>" + "</div>" + "</div>");
					...
					<div ng-controller="serviceCtrl">
						...
						<div class="table-responsive">
							<table class="table table-hover table-alignment">
								<thead>
									<tr  style="background-color:#24659F;color:#ffff;">
										<th>#</th>
										<th>Issue Id</th>
										<th>Device Name</th>
										<th>Raised Date</th>
								...
								<tbody  ng-repeat="store in myWelcome track by $index">
									...
									<!--  	<td align="left"><span ng-bind="store.textarea"></span></td>-->
										...
										<td align="left">
											...
											<div ng-controller="childCtrl">
												<button ng-click="myFunction(data=store.id)"
													data-toggle="modal" data-target="#collapseOne{{$index}}" 
													class="btn btn-success " data-toggle="collapse"
													style="background-color:#91191c;box-shadow: 0px 0px 2px 2px rgba(0,0,0,0.10);border:0;">
													...
													<div class="modal-dialog">
														<div class="modal-content" style="background-color: #eee">
															<div class="modal-header" style="background-color:#235E93;color:#ffff ">
                                                             <button type="button" class="pull-right" id="userclose"data-dismiss="modal" aria-label="Close" style="background:none;border:0" onclick="onModalclose()" >
                                                              <span aria-hidden="true" class="glyphicon glyphicon-remove"></span></button>
																...
																<div class="table-responsive">
																	...
																	<div ng-if="store.status === 'Open'">
																		<button type="button" class="btn btn-info"
																			...
																			data-target="#demo{{store.ticketId}}" style="background-color:#24659F;box-shadow: 0px 0px 2px 2px rgba(0,0,0,0.10);border:0;">Resolve</button>
																	</div>
																	...
																	<div id="demo{{store.ticketId}}" class="collapse">
																		...
																		<form action="resolveComplaint" method="POST">
																			...
																			<br /> <input type="hidden" class="form-control"
																				ng-value="myVar" name="id"> Comment :
																			<textarea class="form-control" required maxlength="250"
																				name="comment" id="comment1"></textarea>
																			<br /> <input type="submit" value="Submit"
																				class="btn btn-success" style="background-color:#64B246;box-shadow: 0px 0px 2px 2px rgba(0,0,0,0.10);border:0;"> <br />
																			<br />
