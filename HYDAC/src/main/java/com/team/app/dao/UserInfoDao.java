...
public interface UserInfoDao extends JpaRepository<TblUserInfo, Serializable> {
	...
	@Query("Select u From TblUserInfo u where u.orgId=:orgId and u.roleBean.type='usr' and u.appId=:appId ")
	List<TblUserInfo> getUserListByorgappAndType(@Param("orgId") int orgId,@Param("appId") int appId);

	
	@Query("Select u From TblUserInfo u where u.orgId=:orgId and u.roleBean.type='usr'")
	List<TblUserInfo> getUserListByorgAndType(@Param("orgId") int orgId);

	@Query("Select u From TblUserInfo u, Bills b where u.orgId=:orgId and u.roleBean.type='usr'and b.payment_status='Paid'")
	List<TblUserInfo> getUserListByorgAndPaymentStatus(@Param("orgId") int orgId);
